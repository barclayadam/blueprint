    ...
    public class HttpRouteMessagePopulationSource : IMessagePopulationSource
    {
        ...
        public void Build(IReadOnlyCollection<PropertyInfo> ownedProperties, MiddlewareBuilderContext context)
        {
            ...
            foreach (var routePropertyPlaceholder in placeholderProperties)
            {
                ...
                var routeDataVariable = $"{operationContextVariable}.{nameof(ApiOperationContext.RouteData)}";
                ...
                if (inAllRoutes)
                {
                    var fromRouteData = $"{routeDataVariable}[\"{routeProperty.Name}\"]";
                ...
                else
                {
                    ...
                    var tryConversionExpression =
                        $"{routeDataVariable}.{nameof(ApiOperationContext.RouteData.TryGetValue)}(\"{routeProperty.Name}\", out var {outVariableName}) ? " +
