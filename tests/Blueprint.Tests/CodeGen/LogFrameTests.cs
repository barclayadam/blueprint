...
public class LogFrameTests
{
    ...
    public async Task When_LogFrame_Uses_ExceptionVariable_And_No_Params(LogFrameTestCase testCase)
    {
        ...
        {
            m.Frames.Append(testCase.CreateFrame(m.FindVariable(typeof(ApiException)), new EventId(1, "Example Event Id"), "Unhandled exception with message"));
        });
    ...
    public async Task When_LogFrame_Uses_ExceptionVariable_And_Params(LogFrameTestCase testCase)
    {
        ...
        var result = CodegenScenario.ForAction<ApiException, ILogger>((t, m) =>
        {
            m.Frames.Append(testCase.CreateFrame(m.FindVariable(typeof(ApiException)), new EventId(1, "Example Event Id"), "Unhandled exception with message {Message}", new object[] { "The static string message" }));
        });
    ...
    public async Task When_LogFrame_Has_No_Params(LogFrameTestCase testCase)
    {
        ...
        {
            m.Frames.Append(testCase.CreateFrame(new EventId(1, "Example Event Id"), "A static log frame message"));
        });
    ...
    public async Task When_LogFrame_Has_Static_String_Param(LogFrameTestCase testCase)
    {
        ...
        var result = CodegenScenario.ForAction<ILogger>((t, m) =>
        {
            m.Frames.Append(testCase.CreateFrame(new EventId(1, "Example Event Id"), "A dynamic log frame message {WithParam}", new object[] { "The param text" }));
        });
    ...
    public async Task When_LogFrame_Has_Dynamic_Variable_Param(LogFrameTestCase testCase)
    {
        ...
        {
            m.Frames.Append(testCase.CreateFrame(new EventId(1, "Example Event Id"), "A dynamic log frame message {WithParam}", new object[] { m.FindVariable(typeof(string)) }));
        });
